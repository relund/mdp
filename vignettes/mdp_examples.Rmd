...
$$
Second, the parameter `times} denote an upper bound on the number of iterations.
...
for (i in iterations) {
	...
	res[3]<-round(a$reward[1],2)
	res[4]<-round(a$reward[1]-termValues[1],2)
	res[5]<-a$actionLabel[2]
	res[6]<-round(a$reward[2],2)
	res[7]<-round(a$reward[2]-termValues[2],2)
	res[8]<-a$actionLabel[3]
	res[9]<-round(a$reward[3],2)
	res[10]<-round(a$reward[3]-termValues[3],2)
	df[i,]<-res
	termValues<-a$reward
}
...
policy<-getPolicy(mdp)
rpo<-calcRPO(mdp, wLbl, iA=0, criterion="average", dur=durLbl, rate=rate, rateBase=rateBase)
policy<-merge(policy,rpo)
policy
```
