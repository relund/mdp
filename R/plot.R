...
plotHypergraph<-function(gridDim,states=NULL,actions=NULL,showGrid=FALSE,fileN=NULL,devOff=TRUE){
   ...
   if (showGrid) {
      for (i in 1:dim(pos)[1]) textrect(pos[i, ], lab = i, cex = 1, radx = 0.0)
   }
   
   # plot states
   if (!is.null(states)) {
      for (i in 1:length(states$gId)) { 
         textellipse(pos[states$gId[i], ], lab = states$label[i], cex = 0.5, radx = 0.02, rady=0.03, shadow.size = 0, lwd=0.5) 
      }
   }
   if (devOff) dev.off()
}