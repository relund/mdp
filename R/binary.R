...
#' \itemize{
#'   \item{`setWeights(labels, ...)`: }{Set the labels of the weights used in the actions. 
#'   `labels` is a vector of label names, `...` are not used. The function must be called
#'   before starting building the model.}
#'   
#'   \item{`process()`: }{Starts a (sub)process. May also be used to specify a traditional MDP 
#'   using matrices (package `MDPtoolbox` style). The style is as follows: P is a list of matrices (one for 
...
#'   
#'   \item{`stage(label=NULL)`: }{Starts a stage. Currently `label` are not used in the
#'   binary format.}
#'   
#'   \item{`endStage()`: }{Ends a (sub)process.}
#'   
#'   \item{`state(label=NULL)`: }{Starts a state. Returns (invisible) the states id number
#'   which can be used for later reference given scope 3.}
#'   
#'   \item{`endState()`: }{Ends a stage.}
#'   
#'   \item{`action(scope=NULL, id=NULL, pr=NULL, prob=NULL, weights, label=NULL, end=FALSE, ...)`: }
#'   {Starts an action. Parameter
#'   `weights` must be a vector of action weights. There are two ways to enter transition prob 
#'   1) `prob` contains triples of (scope,id,pr), 
#'   2) Vectors `id` and `pr` are of equal size. If `scope` not is specified, all scopes are assumed 1.
#'    (see the description of `actionIdx.bin` below). If `end=TRUE` then an `endAction()` is not necessary. 
#'   `...` is currently not used.}
#'   
#'   \item{`endAction()`: }{Ends an action. Do not use if you set `end=TRUE` when you specify an action.}
#'   
#'   \item{`includeProcess(prefix, label=NULL, weights, prob, termStates)`: }{Include an 
...
#'   Parameter `prefix` is the prefix of the external process. The next parameters specify the
#'   child jump action to the process, i.e. `weights` must be a vector of action weights,
#'   `prob` must contain triples of `(scope, idx, pr)` (see the description of `actionIdx.bin`
#'   below), Finally `termStates` must specify the number of states at the last stage in the
#'   external process. Note that inside an `includeProcess ... endIncludeProcess` you must
...
#'   
#'   \item{`endIncludeProcess()`: }{Ends an `includeProcess`.}
#'   
#'   \item{`closeWriter()`: }{Close the writer. Must be called when the model description has
#'   finished.}}
#'
...
#' weights in the format `label1 label2 label3` assuming three weights for each action. }
...
#' \item{`externalProcesses.bin`: }{File of
...
#'   \item{`setWeights(labels, ...)`: }{Set the labels of the weights used in the actions.
#'      `labels` is a vector of label names, `...` are not used.
#'      The function must be called before starting building the model.}
#'   \item{`addAction(label=NULL, sIdx, weights, prob, ...)`: }{Add an action. Parameter 
#'      `sIdx` is the id of the state defining the action, `weights` must be a vector of 
#'      action weights,
#'      `prob` is a matrix `(sIdx,pr)`  where the first column contain the id of the transition 
#'      state (see the description of `actionIdx.bin` below - scope is assumed to the 3), `...` 
#'      is currently not used.}
