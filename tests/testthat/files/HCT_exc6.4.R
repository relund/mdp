...
w <- binary_mdp_writer("hct64_", get_log = FALSE)
w$set_weights(c("Duration", "Net reward"))
w$process()
w$stage()
w$state(label = 0)
w$action(label = a0, weights = c(1, -cost(0, 0)), prob = trans_pr(0, 0), end = T)
w$end_state()
for (i in 1:4) {
  w$state(label = i)
  w$action(label = a0, weights = c(1, -cost(i, 0)), prob = trans_pr(i, 0), end = T)
  w$action(label = a1, weights = c(1, -cost(i, 1)), prob = trans_pr(i, 1), end = T)
  w$end_state()
}
w$end_stage()
w$end_process()
