...
N <- 5
Cf <- -10
Cp <- c(0, -7, -7, -5) # use negative numbers since optimize reward
Q <- matrix(c(
  0.90, 0.10, 0, 0, 0,
...
get_trans_pr <- function(a, i) {
  if (a == 0) {
    return(transPr0$pr[transPr0$i == i])
  }
  if (a == 1) {
    return(transPr1$pr[transPr1$i == i])
  }
  return(stop("Error"))
}
get_idx <- function(a, i) {
  if (a == 0) {
    return(transPr0$j[transPr0$i == i])
  }
  if (a == 1) {
    return(transPr1$j[transPr1$i == i])
  }
  return(stop("Error"))
}

w <- binary_mdp_writer("hct611v2_", get_log = FALSE)
w$set_weights(c("Duration", "Net reward"))
w$process()
w$stage()
w$state(label = "i=1")
w$action(label = "no repair", weights = c(1, 0), id = get_idx(a = 0, i = 1), pr = get_trans_pr(a = 0, i = 1), end = T)
w$end_state()
for (ii in 2:(N - 1)) {
  w$state(label = paste("i=", ii, sep = ""))
  w$action(label = "no repair", weights = c(1, 0), id = get_idx(a = 0, i = ii), pr = get_trans_pr(a = 0, i = ii), end = T)
  w$action(label = "preventive repair", weights = c(1, Cp[ii]), id = get_idx(a = 1, i = ii), pr = get_trans_pr(a = 1, i = ii), end = T)
  w$end_state()
}
