...
N <- 5
Cf <- -10
Cp <- c(NA, -7, -7, -5) # use negative numbers since optimize reward
Q <- matrix(c(
  0.90, 0.10, 0, 0, 0,
...
## Using binary_mdp_writer nested style
...
get_trans_pr <- function(a, i) {
  if (a == 0) {
    return(as.vector(as.matrix(t(transPr0[transPr0$i == i, c(2, 3, 4)]))))
  }
  if (a == 1) {
    return(as.vector(as.matrix(t(transPr1[transPr1$i == i, c(2, 3, 4)]))))
  }
  if (a == 2) {
    return(as.vector(as.matrix(t(transPr2[transPr2$i == i, c(2, 3, 4)]))))
  }
  return(stop("Error"))
}
# Build the model which is stored in a set of binary files
w <- binary_mdp_writer("hct611v3-1_", get_log = FALSE)
w$set_weights(c("Duration", "Net reward"))
w$process()
w$stage()
w$state(label = "i=1")
w$action(label = "no repair", weights = c(1, 0), prob = get_trans_pr(a = 0, i = 1), end = T)
w$end_state()
for (ii in 2:(N - 1)) {
  w$state(label = paste("i=", ii, sep = ""))
  w$action(label = "no repair", weights = c(1, 0), prob = get_trans_pr(a = 0, i = ii), end = T)
  w$action(label = "preventive repair", weights = c(1, Cp[ii]), prob = get_trans_pr(a = 1, i = ii), end = T)
  w$end_state()
}
for (ii in N:(N + 1)) {
  w$state(label = paste("i=", ii, sep = ""))
  cost <- ifelse(ii == N, Cf, 0)
  w$action(label = "forced repair", weights = c(1, cost), prob = get_trans_pr(a = 2, i = ii), end = T)
  w$end_state()
}
w$end_stage()
w$end_process()
...
## Using binary_mdp_writer where the MDP is specified using a list of matrices and a matrix of rewards
...
# make bin files
w <- binary_mdp_writer("hct611v3-2_", get_log = FALSE)
w$set_weights(c("Duration", "Net reward"))
w$process(P, R)
w$close_writer()


## Using binary_mdp_writer where the MDP is specified using a list of sparse matrices and a sparse matrix of rewards
...
w <- binary_mdp_writer("hct611v3-3_", get_log = FALSE)
w$set_weights(c("Duration", "Net reward"))
w$process(P, R)
w$close_writer()


## Using binary_mdp_writer where the semi-MDP is specified using a list of matrices, a matrix of
...
D[5, 3] <- 2
# make bin files
w <- binary_mdp_writer("hct611v3-4_", get_log = FALSE)
w$set_weights(c("Duration", "Net reward"))
w$process(P, R, D)
w$close_writer()
