...
trans_pr <- function(a, row_id) {
  if (a == 0) {
    ...
    pr <- a0TransPr[row_id, idx]
    return(pr)
  }
  if (a == 1) {
    idx <- which(a1TransPr[row_id, ] > 0)
    pr <- a1TransPr[row_id, idx]
    return(pr)
  }
  return(NULL)
}

idx <- function(a, row_id) {
  if (a == 0) {
    idx <- which(a0TransPr[row_id, ] > 0)
    return(idx - 1)
  }
...
w <- binary_mdp_writer("hct67v2_", get_log = FALSE)
...
for (ii in 1:length(states$j)) {
  ...
  w$action(label = "0", weights = c(1, -states$a0cost[ii]), pr = trans_pr(0, ii), id = idx(0, ii), end = T)
  w$action(label = "1", weights = c(1, -states$a1cost[ii]), pr = trans_pr(1, ii), id = idx(1, ii), end = T)
  w$end_state()
}
w$end_stage()
w$end_process()
