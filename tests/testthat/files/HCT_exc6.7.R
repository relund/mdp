...
trans_pr <- function(a, row_id) {
  if (a == 0) {
    idx <- which(a0TransPr[row_id, ] > 0)
    pr <- rbind(1, idx - 1, a0TransPr[row_id, idx])
    return(as.vector(pr))
  }
  if (a == 1) {
    idx <- which(a1TransPr[row_id, ] > 0)
    pr <- rbind(1, idx - 1, a1TransPr[row_id, idx])
    return(as.vector(pr))
  }
  return(NULL)
}
w <- binary_mdp_writer("hct67_", get_log = FALSE)
...
for (ii in 1:length(states$j)) {
  ...
  w$action(label = "0", weights = c(1, -states$a0cost[ii]), prob = trans_pr(0, ii), end = T)
  w$action(label = "1", weights = c(1, -states$a1cost[ii]), prob = trans_pr(1, ii), end = T)
  w$end_state()
}
w$end_stage()
w$end_process()
