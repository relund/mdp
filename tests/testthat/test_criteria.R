...
test_that("Total reward",{
   ...
   valueIte(mdp, w, termValues=scrapValues, getLog = FALSE)
   ...
   w<-"Net reward"             # label of the weight we want to optimize
   valueIte(mdp, w, termValues=0, getLog = FALSE)
...
test_that("Long run average reward",{
   ...
   expect_equal(policyIteAve(mdp,"Net reward","Duration", getLog = FALSE), 5.71428571428571441259691710001789033412933349609375)
   expect_equal(policyIteAve(mdp,"Net reward","Items", getLog = FALSE), 4)
   expect_equal(policyIteAve(mdp,"Items","Duration", getLog = FALSE), 2.71428571428571441259691710001789033412933349609375)
   rm(mdp)
   ...
   expect_equal(policyIteAve(mdp,"Net reward","Duration", getLog = FALSE), -17.7686915887850460649133310653269290924072265625)
   rm(mdp)
   
   source("files/HCT_exc6.7.R")
   mdp<-loadMDP("hct67_", getLog = FALSE)
   expect_equal(policyIteAve(mdp,"Net reward","Duration", getLog = FALSE), -7.8132707659357780727304998436011373996734619140625)
   rm(mdp)
   
   source("files/HCT_exc7.3.R")
   mdp<-loadMDP("hct73_", getLog = FALSE)
   expect_equal(policyIteAve(mdp,"Net reward","Duration", getLog = FALSE), 4)
   rm(mdp)
})
...
test_that("Discounted expected reward",{
   mdp<-loadMDP("2lev_", getLog = FALSE)
   rate<-0.1
   policyIteDiscount(mdp, "Net reward", "Duration", rate, getLog = FALSE)
   weightsPolicyIte<-getPolicy(mdp)$weight
   expect_equal(weightsPolicyIte[16], 56.33119951473481279435873148031532764434814453125)
   valueIte(mdp, "Net reward", "Duration", rate, eps = 1e-15, maxIte = 10000, getLog = FALSE)
   ...
   policyIteDiscount(mdp, "Net reward", "Duration", discountFactor = d, getLog = FALSE)
