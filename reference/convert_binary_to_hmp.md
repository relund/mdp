# Convert a HMDP model stored in binary format to a `hmp` (XML) file. The function simply parse the binary files and create `hmp` files using the [`hmp_mdp_writer()`](http://relund.github.io/mdp/reference/hmp_mdp_writer.md).
