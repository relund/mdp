...
``` r
state_idx_mat(prefix = "", file = "stateIdx.bin")
```
...
A matrix with columns `(s_id, n0, s0, a0, ...)` where `s_id` is the
state row id, `n0` the index of the stage at level 0, `s0` the index of
