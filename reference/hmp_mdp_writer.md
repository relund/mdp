...
- rate_base:
  ...
  `rate_base` is 365 days then we have an interest rate of 10 percent
  over the year.
...
- `set_weights(labels, duration)`: sets the labels of the weights used
  in the actions. `labels` is a vector of label names. `duration`
...
- `set_trans_weights(labels)`: sets the labels of transition-level
  weights.
...
- `action(label = NULL, weights, prob, states_next = NULL, trans_weights = NULL)`:
  ...
  format. `states_next` is the number of states in the next stage of the
...
#> <mlhmp l="HMP file created using hmp_mdp_writer in R" b="0.1" dsl="1" precision="1e-05" version="1.1">
#>   <i>0.1</i>
