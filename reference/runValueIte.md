...
- eps:
  ...
  Stopping criterion. If \$max(w(t)-w(t+1)) \< `eps`\$ then stop the
...
- g:
  ...
  Average reward. If specified then do a single iteration using the
  update equations under average reward criterion with the specified g
  value.
...
mdp<-loadMDP(prefix)
#> Read binary files (0.000106726 sec.)
#> Build the HMDP (3.6558e-05 sec.)
#> Checking MDP and found no errors (2.241e-06 sec.)
mdp
#> $binNames
...
#> 
#> $ptr
#> C++ object <0x55a88b3ebc00> of class 'HMDP' <0x55a887db2f30>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
...
#>  Finished. Cpu time 6.178e-06 sec.
...
mdp<-loadMDP(prefix)
#> Read binary files (0.000112448 sec.)
#> Build the HMDP (3.2834e-05 sec.)
#> Checking MDP and found no errors (2.354e-06 sec.)
mdp
#> $binNames
...
#> 
#> $ptr
#> C++ object <0x55a88e364700> of class 'HMDP' <0x55a887db2f30>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
...
#> Run value iteration with epsilon = 0 at most 1 time(s)
#> using quantity 'Net reward' under reward criterion.
#>  Finished. Cpu time 6.607e-06 sec.
getPolicy(mdp)     # optimal policy
#> # A tibble: 13 × 6
#>      sId stateStr stateLabel   aIdx actionLabel weight
#>    <dbl> <chr>    <chr>       <int> <chr>        <dbl>
