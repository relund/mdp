...
- hgf:
  ...
  have columns `sId` (head node), `trans<n>` (tail nodes), `aIdx`
  (action index), `label` (action label), `lwd` (hyperarc line width),
  `lty` (hyperarc line type) and `col` (hyperarc color).
