...
If the policy does not contain all states then the actions from the
previous optimal policy are used.
...
- policy:
  ...
  A data frame with two columns state id `s_id` and action index
  `a_idx`.
...
mdp<-load_mdp(prefix)
#> Read binary files (0.000179624 sec.)
#> Build the HMDP (4.4385e-05 sec.)
#> Checking MDP and found no errors (2.294e-06 sec.)
mdp
#> $bin_names
...
#> 
#> $ptr
#> C++ object <0x556227b8f5d0> of class 'HMDP' <0x55622202d5e0>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
...
#>  Finished. Cpu time 9.354e-06 sec.
...
mdp<-load_mdp(prefix)
#> Read binary files (0.000181086 sec.)
#> Build the HMDP (4.8982e-05 sec.)
#> Checking MDP and found no errors (4.116e-06 sec.)
mdp
#> $bin_names
...
#> 
#> $ptr
#> C++ object <0x55622acf4720> of class 'HMDP' <0x55622202d5e0>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
