...
- prefix:
  ...
  A character string with the prefix added to `bin_names`. Used to
...
- eps:
  ...
  The sum of the transition probabilities must at most differ `eps` from
  one.
...
A list containing relevant information about the model such as model
file names (`bin_names`), time horizon (`time_horizon`), number of
states (`states`), number of states at last stage of the founder process
(`founder_states_last`), number of actions (`actions`), number of levels
(`levels`), names of the weights associated to each action
(`weight_names`) and a pointer `ptr` to the model object in memory. Note
for models with an infinite time-horizon the states at the founder level
is repeated at stage two so have something aka a double array
representation.
...
mdp<-load_mdp(prefix)
#> Read binary files (0.000210098 sec.)
#> Build the HMDP (6.3794e-05 sec.)
#> Checking MDP and found no errors (1.762e-06 sec.)
mdp
#> $bin_names
...
#> 
#> $ptr
#> C++ object <0x556226165b60> of class 'HMDP' <0x55622202d5e0>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
...
#>  Finished. Cpu time 9.494e-06 sec.
...
mdp<-load_mdp(prefix)
#> Read binary files (0.000183379 sec.)
#> Build the HMDP (4.786e-05 sec.)
#> Checking MDP and found no errors (1.633e-06 sec.)
mdp
#> $bin_names
...
#> 
#> $ptr
#> C++ object <0x556226d3fc80> of class 'HMDP' <0x55622202d5e0>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
...
#> Run value iteration with epsilon = 0 at most 1 time(s)
#> using weight 'Net reward' under expected-weight Bellman operator.
#>  Finished. Cpu time 1.1567e-05 sec.
get_policy(mdp)     # optimal policy
#> # A tibble: 13 × 6
#>     s_id state_str state_label a_idx action_label weight
#>    <dbl> <chr>     <chr>       <int> <chr>         <dbl>
