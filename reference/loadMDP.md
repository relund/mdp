...
- prefix:
  ...
  A character string with the prefix added to `binNames`. Used to
...
file names (`binNames`), time horizon (`timeHorizon`), number of states
(`states`), number of states at last stage of the founder process
(`founderStatesLast`), number of actions (`actions`), number of levels
(`levels`), names of the weights associated to each action
(`weightNames`) and a pointer `ptr` to the model object in memory. Note
...
mdp<-loadMDP(prefix)
#> Read binary files (0.000163772 sec.)
#> Build the HMDP (4.1722e-05 sec.)
#> Checking MDP and found no errors (1.122e-06 sec.)
mdp
#> $binNames
...
#> 
#> $ptr
#> C++ object <0x56083c5e56b0> of class 'HMDP' <0x56083a3616c0>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
...
#>  Finished. Cpu time 1.0135e-05 sec.
...
mdp<-loadMDP(prefix)
#> Read binary files (0.000185224 sec.)
#> Build the HMDP (4.4546e-05 sec.)
#> Checking MDP and found no errors (1.873e-06 sec.)
mdp
#> $binNames
...
#> 
#> $ptr
#> C++ object <0x560836fc4b50> of class 'HMDP' <0x56083a3616c0>
#> 
#> attr(,"class")
#> [1] "HMDP" "list"
plot(mdp)
...
#> Run value iteration with epsilon = 0 at most 1 time(s)
#> using weight 'Net reward' under expected-weight Bellman operator.
#>  Finished. Cpu time 1.0325e-05 sec.
getPolicy(mdp)     # optimal policy
#> # A tibble: 13 × 6
#>      sId stateStr stateLabel   aIdx actionLabel weight
#>    <dbl> <chr>    <chr>       <int> <chr>        <dbl>
